...
export function PhoneMockup({ src, caption, variant = "grid" }: PhoneMockupProps) {
  ...
  return (
    <figure className="group flex flex-col items-center gap-3" style={{ margin: 0 }}>
      <div className="relative" style={{ width }}>
        {/* ambient glow */}
        <div
          className="absolute -inset-6 -z-10 rounded-[3rem] opacity-70 blur-2xl"
          style={{ background: "radial-gradient(60% 55% at 50% 35%, #eff4ff, transparent 70%)" }}
        />
        {/* device frame */}
        <div
        ...
        >
          <div className="relative overflow-hidden rounded-[1.6rem] bg-neutral-50">
            ...
            <img src={src} alt={caption ?? ""} className="block w-full object-cover" loading="eager" decoding="async" />
          </div>
        </div>
      </div>
      {caption && (
